...
double Function :: Eval(double x) {
    return x; //per la stima dell'integrale
}
double Function :: EvalErr(double x){
    return pow( ( x - 0.5 ), 2 ); //per la stima di sigma^2
}
...
void BlockAverage :: Average(int iblock, int L, ofstream &Average, ofstream &Error){
    stima = blk_av[iav]/blk_norm;
    glob_av[iav] += stima;
    glob_av2[iav] += stima*stima;
    errore = error(glob_av[iav], glob_av2[iav], iblock);

    Average << iblock*L << "\t" << glob_av[iav]/(double)iblock << "\t" << errore << endl;

    stima = blk_av[ierr]/blk_norm;
    glob_av[ierr] += stima;
    glob_av2[ierr] += stima*stima;
    errore = error(glob_av[ierr], glob_av2[ierr], iblock);

    Error << iblock*L << "\t" << glob_av[ierr]/(double)iblock << "\t" << errore << endl;
