...
int main(int argc, char** argv) //passata la fase del sistema da terminale
{ 

	if(argc != 2)
  {
    cerr << "Use: " << argv[0]  << ", phase of the system" << endl;
    return -1;
  }
...
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! 
    ...
    Averages(iblk, phase);   //Print results for current block
  }
...
//Read initial configuration
  ...
  if(restart) //if true, i.e. restart == 1
  {
    //cout << "qui" << endl;
...
void Measure() //Properties measurement
{
  ...
  //Inizializzati a zero tutti i bin della g(r)
  ...
  // distance i-j in pbc
      ...
      //incremento dei bin se dr sta nel range [0, L/2]
      if(dr < box/2.) {
  ...
  walker[iv] = 4.0 * v + (double)npart*( 8.*M_PI*rho )*( 1./( 9.* pow( rcut, 9 ) ) - 1./( 3.* pow( rcut, 3 ) ) ); // Potential energy with tail corrections
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin;  // Total energy;
	walker[ipr] = rho*walker[it] + 48.*pressure/(3.*vol) + 3.*double(npart)*32.*M_PI*( rho/vol )*( 1./( 9.*pow( rcut, 9 ) ) - 1./( 6.*pow( rcut, 3 ) ) ); //Pressure with tail corrections
  return;
}
...
void Averages(int iblk, string phase) //Print results for current block
{
    
    ofstream Epot, Press, Gr, Gr_final;
    const int wd=12;
    
    //cout << "Block number " << iblk << endl;
    //cout << "Acceptance rate " << accepted/attempted << endl << endl;
    
    Epot.open(phase + "/output_epot_" + simulation, ios::app);
		Press.open(phase + "/output_press_" + simulation, ios::app);
    Gr.open(phase + "/output_gr_" + simulation, ios::app);
    Gr_final.open(phase + "/output_gr_final_" + simulation, ios::app);
    ...
    err_pot=Error(glob_av[iv],glob_av2[iv],iblk);
		...
		stima_pr = blk_av[ipr]/blk_norm; //Pressure
    glob_av[ipr] += stima_pr;
    glob_av2[ipr] += stima_pr*stima_pr;
    err_pr=Error(glob_av[ipr],glob_av2[ipr],iblk);

    //Calcolo della g(r)
    for(int i = n_props; i < nbins + n_props; i++) {
      //cout << "qui" << endl;
      norm = rho * npart * (4. * M_PI)/3. * ( pow( bin_size*( i - n_props ) + bin_size, 3) - pow(bin_size*( i - n_props ), 3) ); //i - n_props indicizza il bin, bin_size = dr
      stima_gr = blk_av[i]/( (double)blk_norm * norm );
      glob_av[i] += stima_gr;
      glob_av2[i] += pow( stima_gr, 2 );
      err_gr = Error(glob_av[i], glob_av2[i], iblk);
      Gr << setw(wd) << iblk <<  setw(wd) << stima_gr << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gr << endl;

      if( iblk == nblk ) {
        Gr_final << setw(wd) << iblk << setw(wd) << bin_size*( i - n_props ) << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gr << endl;
      }
    }

		//'if' to print the single block value during equilibration
		if(restart == 0) {
			//Potential energy per particle
	    Epot << setw(wd) << iblk <<  setw(wd) << stima_pot << endl;
		  //Pressure
			Press << setw(wd) << iblk <<  setw(wd) << stima_pr << endl;
		}
	    ...
	    Epot << setw(wd) << iblk <<  setw(wd) << stima_pot << setw(wd) << glob_av[iv]/(double)iblk << setw(wd) << err_pot << endl;
		  //Pressure
			Press << setw(wd) << iblk <<  setw(wd) << stima_pr << setw(wd) << glob_av[ipr]/(double)iblk << setw(wd) << err_pr << endl;
		}
    //cout << "----------------------------" << endl << endl;
