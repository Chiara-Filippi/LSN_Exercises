...
// distance i-j in pbc
  ...
  walker[ie] = 4.0 * v + kin;  // Total energy;
	walker[ipr] = rho*walker[it] + 48.*pressure/(3.*vol); 
  return;
}
...
void Averages(int iblk, string phase) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open(phase + "/output_epot", ios::app);
    Ekin.open(phase + "/output_ekin", ios::app);
    if(!restart) {
      Temp.open(phase + "/output_temp_termalization", ios::app);
    }
    else {
      Temp.open(phase + "/output_temp", ios::app);
    }
    Etot.open(phase + "/output_etot", ios::app);
		Press.open(phase + "/output_press", ios::app); //aggiungi pressione
	...
	//Kinetic energy
	    Ekin << setw(wd) << iblk <<  setw(wd) << stima_kin << endl;
	//Total energy
	    Etot << setw(wd) << iblk <<  setw(wd) << stima_etot << endl;
	//Temperature
	    Temp << setw(wd) << iblk <<  setw(wd) << stima_temp << endl;
		//Pressure
