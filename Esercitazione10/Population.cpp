...
void RandomInitializer(Random& rnd, int rank){
    int seed[4];
    int p1, p2;
    ...
    if (input.is_open()){
        while ( !input.eof() ){
            ...
            if( property == "RANDOMSEED" ){
                ...
                seed[3] += rank + 1; //seed diverso per ogni processore
...
//Costruttore
Population::Population(Random& r, int npopulation, int ncities, Map& map){
    rnd = r;
    //Generata la mappa
    m_map = map;
...
double Population::RouteLength(int iroute){
...
Route Population::GetBestPopulation(){
    return m_pop[0];
}

void Population::SetIndividual(Route& best, int i){
    m_pop[i] = best;
}
